...
-- tracking set, custom add/remove, the UNIT_AURA detection path (added auras,
-- self-cast exclusion, the in-combat "secret value" graceful path, the per-spell
-- throttle, removal hiding the alert), TTS + sound output, the options window
-- construction, and the slash command. The aura source (caster) is deliberately
-- never named -- see the module header.
...
C_UnitAuras = {}   -- present but unused (we drive UNIT_AURA payloads directly)
...
-- helper: build a fresh UNIT_AURA "added" payload for one aura
local function aura(spellId, fields)
...
spoken = {}
clock = clock + 100
added(aura(1044, { isFromPlayerOrPlayerPet = true }))
check("self-cast (player/pet) excluded", #spoken == 0)

-- self-cast via sourceUnit == player excluded
spoken = {}
clock = clock + 100
added(aura(1044, { sourceUnit = "player" }))
check("self-cast (sourceUnit player) excluded", #spoken == 0)

-- ===========================================================================
-- The in-combat "secret value" reality: caster unreadable, alert still fires
-- ===========================================================================
local SECRET_SRC = setmetatable({}, { __tostring = function() return "secret" end })
secretVals[SECRET_SRC] = true
spoken = {}
clock = clock + 100
added(aura(102342, { sourceUnit = SECRET_SRC, isFromPlayerOrPlayerPet = SECRET_SRC }))
check("secret sourceUnit still alerts (graceful)", #spoken == 1 and spoken[1].text == "Ironbark")
secretVals[SECRET_SRC] = nil

-- a secret spellId is skipped (can't branch on it)
local SECRET_ID = setmetatable({}, {})
secretVals[SECRET_ID] = true
spoken = {}
clock = clock + 100
added(aura(SECRET_ID))
check("secret spellId skipped without error", #spoken == 0)
secretVals[SECRET_ID] = nil

-- ===========================================================================
-- Per-spell throttle (db.throttle seconds)
-- ===========================================================================
spoken = {}
clock = clock + 100
added(aura(6940))                       -- first time -> speaks
added(aura(6940))                       -- immediately again -> throttled
...
E.db.throttle = 0
spoken = {}
added(aura(31821)); added(aura(31821))
...
-- ===========================================================================
-- Removal hides the showing alert early
-- ===========================================================================
clock = clock + 100
added(aura(414660, { auraInstanceID = 4242 }))
check("alert showing before removal", E.alert:IsShown() == true)
E:UNIT_AURA("player", { removedAuraInstanceIDs = { 4242 } })
check("matching removal hides the alert", E.alert:IsShown() == false)

-- a non-matching removal leaves an alert alone
clock = clock + 100
added(aura(374227, { auraInstanceID = 7 }))
E:UNIT_AURA("player", { removedAuraInstanceIDs = { 9999 } })
check("non-matching removal leaves the alert", E.alert:IsShown() == true)
...
added(aura(33206, { duration = 8, expirationTime = clock + 6 }))
...
added(aura(102342, { duration = SECRET_DUR, expirationTime = SECRET_EXP }))   -- Ironbark, table dur 12
...
E:HideAlert(); clock = clock + 100
added(aura(145629, { duration = 0, expirationTime = 0 }))   -- Anti-Magic Zone standing-in aura
...
check("non-player unit is ignored", pcall(function() E:UNIT_AURA("target", { addedAuras = { aura(1022) } }) end))
check("empty addedAuras is safe", pcall(function() E:UNIT_AURA("player", { addedAuras = {} }) end))
